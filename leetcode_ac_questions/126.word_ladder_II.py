...
class Solution(object):
    def findLadders(self, beginWord, endWord, wordList):
        ...
        :rtype: List[List[str]]
        """
        word_graph = self.construct_graph(beginWord, wordList)

        p, final_depth = self.bfs(beginWord, endWord, word_graph)

        result_lists = []
        self.dfs(result_lists, endWord, beginWord, final_depth, p, result_list=None)

        return [items[::-1] for items in result_lists]
            ...
            for w3 in wordList:
                ...
                for i in range(0, len(beginWord)):
                    if beginWord[i] != w3[i]:
                        count += 1
                if count == 1:
                    word_graph[beginWord].append(w3)

        return word_graph

    def bfs(self, beginWord, endWord, word_graph):
        q, p, w, s, final_depth = deque([beginWord]), defaultdict(list), defaultdict(int), set(beginWord), None
        p.setdefault(beginWord, [])
        w.setdefault(beginWord, 0)

        while q:
            u = q.popleft()

            for v in word_graph[u]:
                if v in s:
                    if v in w and u in w and u not in p[v] and w[v] == w[u] + 1:
                        p[v].append(u)
                    continue
                ...
                if u not in p[v]:
                    p[v].append(u)
                w.setdefault(v, w[u] + 1)
                s.add(v)

        return p, final_depth

    def dfs(self, result_lists, beginWord, endWord, final_depth, word_graph, result_list=None):
        if not result_list:
            result_list = []
        ...
        if beginWord == endWord and final_depth == 0:
            result_list.append(beginWord)
            result_lists.append(copy.deepcopy(result_list))
            result_list.pop()
            return

        result_list.append(beginWord)

        for v in word_graph[beginWord]:
            self.dfs(result_lists, v, endWord, final_depth - 1, word_graph, result_list)
